	...
	customElements.define(
		...
		class DateRangePicker extends HTMLElement {
			...
			monthsEventListeners() {
				...
				months.forEach((month, index) => {
					...
					month.addEventListener("mouseover", function () {
						...
						if (widget.sm == undefined ) { //if nothing was previously selected (no valid selection)
			...
			setInitialVisibleYear(initialYear) {
				...
				if(initialYear!=='' && initialYear!==undefined){
			...
			setStartDate(startDateYear, startDateMonth) {
				...
				startDateYear = startDateYear==undefined?'':startDateYear;
				startDateMonth = startDateMonth==undefined?'':startDateMonth;
				
				if(startDateYear!==''){
					if(this.calendarType=='month' && startDateMonth==''){
						startDateMonth='1';
					}
			...
			setEndDate(endDateYear, endDateMonth) {
				...
				endDateYear = endDateYear==undefined?'':endDateYear;
				endDateMonth = endDateMonth==undefined?'':endDateMonth;
				
				if(endDateYear!==''){
					if(this.calendarType=='month' && endDateMonth==''){
						endDateMonth='12';
					}
			...
			setVisibleRange(yearFrom, monthFrom, yearTo, monthTo) {
					...
					if(this.calendarType==='month'){ //this will never happen, as validation will not allow it
