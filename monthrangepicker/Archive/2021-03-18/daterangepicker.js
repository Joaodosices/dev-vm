	...
	customElements.define(
		...
		class DateRangePicker extends HTMLElement {
			...
			setDecadeGrid(init) {
				...
				this.decadeLabel.innerHTML = `${drawForYear - this.yearsToCentury}-${this.initDYear + 11 - this.yearsToCentury}`;
    ...
    yearsEventListeners() { 
      ...
      yearsBtnListeners() {
        ...
        widget.dNext.addEventListener("click", function () {
          widget.currentYear = widget.currentYear + 12;
          widget.setDecadeGrid();
        });
      ...
      formatDate(month, year, format) {
        ...
        return returnDate;
			...
			setDisabledPeriod(year, month) {
				...
				////console.log(this.restrictedPeriods["m".concat(disabledPeriod)]);
			}
			...
			isDisabled(selectedYear, selectedMonth){
				...
				////console.log(this.restrictedPeriods);
				////console.log('m'.concat(selectedYear.toString()).concat(selectedMonth.toString()));
		...
		if(this.calendarType=='month'){
			...
			/*
			if (this.em!==undefined) { //selection is finished as end month is selected
			  if (this.currentYear == this.sy || this.currentYear == this.ey) { //current year is year from or year to
				if (this.ey == this.sy) { //end year and selected year is the same
				  for (let i = 0; i < 12; i++) {
						////console.log(this.whatClass(this.currentYear, i+1));
					if (this.isDisabled(this.currentYear, i+1)){
						this.months[i].className = "month disabled";
					} else {
